 ...
 * {@link com.rio.chequeo.servlet.PDFServlet}.
 * El archivo web.xml registra la clase {@code com.rio.chequeo.servlet.PDFServlet}.</p>
 *
 * <ul>
...
public class PDFServlet extends HttpServlet {
    ...
    private void handlePreview(HttpServletRequest req, HttpServletResponse resp, String previewStr)
        ...
        if (sheetNumber < 1 || sheetNumber > 26) {
            resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "preview debe ser entre 1 y 26");
            return;
        }
        ...
        resp.setHeader("Content-Disposition",
                "inline; filename=\"preview-sheet-" + sheetNumber + ".pdf\"");
        resp.setContentLength(pdfBytes.length);
        try (OutputStream out = resp.getOutputStream()) {
            out.write(pdfBytes);
        }
    ...
    private SheetData buildSingleSheetData(HttpServletRequest req, HttpServletResponse resp,
        ...
        if (isCoverSheet(sheetNumber)) {
            ...
            sd.htmlContent = (html != null) ? html :
                    "<html><body><p>Hoja " + sheetNumber + "</p></body></html>";
        }
        return sd;
    }
    ...
    private String renderJspToString(HttpServletRequest req, HttpServletResponse resp, String jspPath)
        ...
        ResponseWrapper wrapper = new ResponseWrapper(resp, pw);
    ...
    private byte[] getMembreteImage(int sheet, MedicalRecord r) {
        switch (sheet) {
            ...
            case 22: return r.getMembrete21();
    ...
    // -------------------------------------------------------------------------
    // ResponseWrapper — captura la salida del JSP en memoria
    // -------------------------------------------------------------------------
    ...
    private static final class ResponseWrapper extends javax.servlet.http.HttpServletResponseWrapper {
        ...
        ResponseWrapper(HttpServletResponse response, PrintWriter printWriter) {
            super(response);
